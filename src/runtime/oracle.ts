...
export class Oracle implements IOracle {
  ...
  async chat(message: string, extraUsage?: UsageMetadata, isTelephonist?: boolean, taskContext?: OracleTaskContext): Promise<string> {
    ...
    try {
      ...
      if (messageSource) {
        (userMessage as any).source_metadata = { source: messageSource };
      }
...
8. **NEVER invent, guess, or write a task ID in plain text.** Task IDs are UUIDs generated by the system only when you call a delegation tool (e.g. \`neo_delegate\`, \`apoc_delegate\`). Writing something like "Task ABC123 queued for Neo execution" without actually calling the tool is strictly forbidden — the system will detect and block it.
9. **To create a task you MUST call the delegation tool.** Describing a delegation in text (e.g. "• delegate to neo_delegate with task X") does NOT create a task. Only calling the tool creates a real task with a real ID.
10. Keep responses concise and objective.
11. Avoid duplicate delegations to the same tool or agent.
12. After enqueuing all required delegated tasks for the current message, stop calling tools and return a concise acknowledgement.
13. If a delegation is rejected as "not atomic", immediately split into smaller delegations and retry.
14. When the user message contains @link, @neo, @apoc, or @trinity (case-insensitive), delegate to that specific agent. The mention is an explicit routing directive — respect it even if another agent might also handle the request.
15. Smiths also have names and could be called by @smithname — respect this as an explicit routing directive as well.
16. Delegate Google Workspace (GWS) tasks (Sheets, Docs, Calendar, Drive, Gmail) to the Apoc agent using the \`apoc_delegate\` tool.
