...
export class SatiMemoryMiddleware {
    ...
    async beforeAgent(currentMessage: string, history: BaseMessage[]): Promise<SystemMessage | null> {
        try {
            ...
            if (result.relevant_memories.length === 0) return null;
            
            const memoryContext = result.relevant_memories
                .map(m => `- [${m.category.toUpperCase()}] ${m.summary}`)
                .join('\n');

            display.log('Sati memory retrieved: ' + memoryContext, { source: 'Sati' });
