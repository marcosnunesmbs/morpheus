  ...
  getGlobalSummary(): GlobalAuditSummary {
    ...
    const totalsRow = this.db.prepare(`
      SELECT
        ...
        SUM(CASE WHEN ae.event_type = 'link_search'     THEN 1 ELSE 0 END) as linkSearchCount,
        SUM(CASE WHEN ae.event_type = 'link_index'      THEN 1 ELSE 0 END) as linkIndexCount,
    ...
    return {
      ...
      totals: {
        ...
        linkSearchCount:     totalsRow?.linkSearchCount     ?? 0,
        linkIndexCount:      totalsRow?.linkIndexCount      ?? 0,
      },
      byAgent:   byAgentRows,
      byModel:   byModelRows,
