...
- [x] 2.1 Update `src/config/manager.ts` to add `getSantiConfig()` method which falls back to `this.config.santi` or generic LLM config with defaults
- [x] 2.2 Update `src/runtime/memory/sati/service.ts` to reuse `getSantiConfig()` instead of `getLLMConfig()`
- [x] 2.3 Verify `SatiService` uses the `memory_limit` from the config during retrieval

## 3. CLI Initialization

- [x] 3.1 Update `src/cli/commands/init.ts` to add "Configure Santi (Memory) Agent" step
